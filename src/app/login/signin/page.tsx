...
function SignInForm() {
  ...
  const inp: React.CSSProperties = {
    ...
    borderRadius: 14, padding: "16px 20px", fontSize: 16, color: "rgba(255,255,255,0.9)",
    outline: "none", fontFamily: "inherit", transition: "all 0.2s", boxSizing: "border-box",
  };

  return (
    <div style={{ minHeight: "100vh", background: "#020812", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
      ...
      {/* Back to landing */}
      <button onClick={() => router.push("/login")} style={{ position: "absolute", top: 28, left: 40, display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 14, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      <div style={{ position: "absolute", top: 24, left: "50%", transform: "translateX(-50%)", display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 700, color: "white" }}>InterviewCoach</span>
      </div>

      {/* Form card */}
      <div style={{ width: "100%", maxWidth: 600, padding: "0 24px", position: "relative", zIndex: 5 }}>
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 32, padding: "60px 64px", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)" }}>
          <h2 style={{ fontSize: 30, fontWeight: 700, color: "white", letterSpacing: "-0.03em", marginBottom: 8 }}>
          ...
          {/* Google Sign In */}
          <a href="/api/auth/google" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "16px", borderRadius: 16, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)", color: "white", fontSize: 16, fontWeight: 600, textDecoration: "none", fontFamily: "inherit", transition: "background 0.2s", marginBottom: 8 }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, margin: "8px 0 4px" }}>
            ...
            <span style={{ fontSize: 13, color: "rgba(255,255,255,0.25)", fontWeight: 500 }}>or</span>
            <div style={{ flex: 1, height: 1, background: "rgba(255,255,255,0.07)" }} />
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            {isRegister && (
              <div>
                <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.4)", marginBottom: 8, letterSpacing: "0.05em", textTransform: "uppercase" }}>Name</label>
          ...
          <div style={{ marginTop: 28, paddingTop: 28, borderTop: "1px solid rgba(255,255,255,0.06)", textAlign: "center" }}>
            <button onClick={() => { setIsRegister(!isRegister); setError(""); }} style={{ background: "none", border: "none", fontSize: 15, color: "rgba(255,255,255,0.35)", cursor: "pointer", fontFamily: "inherit" }}>
      ...
      <style>{`input::placeholder { color: rgba(255,255,255,0.22); } @keyframes spin { to { transform: rotate(360deg); } }`}</style>
    </div>
  );
}
